...
export class AuthController {
  ...
  async googleCallback(
    ...
    if (error) {
      response.redirect(this.authService.googleFailureUrl('Google sign-in was cancelled.'));
      return;
    }

    if (!code || !state) {
      response.redirect(this.authService.googleFailureUrl('Google sign-in did not complete.'));
      return;
    }

    if (state !== readCookie(request.headers.cookie, GOOGLE_OAUTH_STATE_COOKIE)) {
      response.redirect(this.authService.googleFailureUrl('Google sign-in state did not match.'));
      return;
    }

    try {
      const session = await this.authService.loginWithGoogleCode(code, state);
      response.redirect(this.authService.googleCallbackUrl(session.token, session.next));
    } catch (caught) {
