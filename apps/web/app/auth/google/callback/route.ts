...
export async function GET(request: Request) {
  ...
  if (
    ...
    const response = NextResponse.redirect(
      publicBrowserUrl(
        "/login?error=Google%20sign-in%20did%20not%20complete.",
        url,
      ),
    );
    response.cookies.delete(GOOGLE_OAUTH_STATE_COOKIE);
  ...
  if (!session) {
    return NextResponse.redirect(
      publicBrowserUrl(
        "/login?error=Google%20sign-in%20session%20expired.",
        url,
      ),
    );
  }

  const response = NextResponse.redirect(publicBrowserUrl(next, url));
