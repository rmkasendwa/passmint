    ...
    <div className="grid grid-cols-[minmax(0,1fr)_420px] items-start gap-[18px] max-[1120px]:grid-cols-1">
        ...
        <aside className="sticky top-[94px] grid gap-[18px] max-[1120px]:static">
          ...
          >
            ...
            {session ? (
              <span className={`inline-flex min-h-[30px] w-fit items-center rounded-full px-2.5 text-[0.78rem] font-(weight:--weight-semibold) uppercase ${session.user.role === "admin" ? "bg-[#ffe6d8] text-[#b43d19]" : "bg-(color:--accent-soft) text-(color:--accent)"}`}>
                {session.user.role}
              </span>
            ) : (
          ...
          <section className={panelPadded}>
            ...
            {selectedEvent && (
              <div className="grid gap-1 rounded-lg bg-(color:--surface-muted) p-3">
                <strong className="text-(color:--text)">{selectedEvent.name}</strong>
                <span className="text-[0.9rem] text-(color:--text-muted)">{dateTime.format(new Date(selectedEvent.startsAt))}</span>
                <span className="text-[0.9rem] text-(color:--text-muted)">{selectedEvent.venue}</span>
              </div>
            )}
          ...
          <section className={panelPadded}>
            ...
            {tickets.length === 0 ? (
              ...
              <div className="grid gap-3">
                {tickets.map((ticket) => (
                  <article className="grid grid-cols-[92px_1fr] items-center gap-3 rounded-lg border border-(color:--border) bg-(color:--surface-muted) p-3 max-[600px]:grid-cols-1" key={ticket.id}>
                    <img
                    ...
                    <div>
                      ...
                      <p className="mb-2 text-(color:--text-muted)">{ticket.event.name}</p>
                      <code className="rounded-md bg-(color:--surface-elevated) px-2 py-1 text-[0.78rem] text-(color:--accent)">{ticket.code}</code>
                    </div>
                  </article>
                ))}
              </div>
              ...
              <div className="grid gap-2.5">
                {ticketHistory.length === 0 ? (
                  ...
                  ticketHistory.map((ticket) => (
                    <article className="flex items-center justify-between gap-3 rounded-lg border border-(color:--border) bg-(color:--surface-muted) p-3" key={ticket.id}>
                      <div>
                        <strong className="block text-(color:--text)">{ticket.event.name}</strong>
                        <small className="text-(color:--text-muted)">
                          {dateTime.format(new Date(ticket.event.startsAt))}
                        </small>
                      </div>
                      <span className={`inline-flex min-h-[28px] items-center rounded-full px-2.5 text-[0.75rem] font-(weight:--weight-semibold) uppercase ${ticket.status === "checked_in" ? "bg-[#dff7e8] text-[#14532d]" : ticket.status === "cancelled" ? "bg-[#ffe8df] text-[#8d2718]" : "bg-(color:--accent-soft) text-(color:--accent)"}`}>
