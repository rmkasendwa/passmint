...
const panel =
  "rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] shadow-[0_18px_44px_rgb(18_24_31/6%)]";
...
const formGrid = "grid gap-3 [&_label]:grid [&_label]:gap-[7px] [&_label]:text-[0.82rem] [&_label]:font-[var(--weight-semibold)] [&_label]:text-[var(--text-muted)] [&_input]:min-h-11 [&_input]:w-full [&_input]:min-w-0 [&_input]:rounded-lg [&_input]:border [&_input]:border-[var(--border)] [&_input]:bg-[var(--surface-elevated)] [&_input]:px-3 [&_input]:text-[var(--text)] [&_input]:focus:border-[var(--accent)] [&_input]:focus:outline-[3px_solid_rgb(22_125_119/18%)]";
const eventCard =
  "grid min-h-[430px] rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] text-left text-[var(--text)] shadow-none hover:border-[var(--border-strong)] [&.selected]:border-[var(--border-strong)] max-[820px]:min-h-[500px] max-[600px]:min-h-0 max-[600px]:grid-rows-[220px_1fr] [&_.event-thumbnail-card]:max-[600px]:min-h-[220px]";
const eventCardCopy =
  "grid grid-rows-[auto_auto_auto_1fr_auto] gap-4 p-[26px] max-[820px]:p-[22px]";
const mutedText = "mb-0 text-[var(--text-muted)]";
    ...
    <div className="grid grid-cols-[minmax(0,1fr)_420px] items-start gap-[18px] max-[1120px]:grid-cols-1">
        <section>
          <div className="mb-[26px] flex items-end justify-between gap-4">
            ...
            <span className="text-base font-[var(--weight-semibold)] text-[var(--text-muted)]">
            ...
            {visibleEvents.map((event, index) => (
              ...
              >
                ...
                <span className={eventCardCopy}>
                  <strong className="text-[1.18rem] font-[var(--weight-bold)] leading-[1.16] text-[var(--text)]">
                    {event.name}
                  </strong>
                  <small className="min-h-[3.1em] text-[1.02rem] font-[var(--weight-regular)] leading-[1.55] text-[var(--text-muted)]">
                    {event.description}
                  </small>
                  <span className="grid gap-2 [&_span]:inline-flex [&_span]:items-center [&_span]:gap-[7px] [&_span]:text-[0.94rem] [&_span]:font-[var(--weight-medium)] [&_span]:leading-[1.34] [&_span]:text-[var(--text-muted)] [&_svg]:text-[var(--accent)]">
                    <span>
                  ...
                  <span className="self-center text-[1.4rem] font-[var(--weight-bold)] text-[#167d77]">
        ...
        <aside className="sticky top-[94px] grid gap-[18px] max-[1120px]:static">
          ...
          >
            ...
            {session ? (
              <span className={`inline-flex min-h-[30px] w-fit items-center rounded-full px-2.5 text-[0.78rem] font-[var(--weight-semibold)] uppercase ${session.user.role === "admin" ? "bg-[#ffe6d8] text-[#b43d19]" : "bg-[var(--accent-soft)] text-[var(--accent)]"}`}>
                {session.user.role}
              </span>
            ) : (
          ...
          <section className={panelPadded}>
            ...
            {selectedEvent && (
              <div className="grid gap-1 rounded-lg bg-[var(--surface-muted)] p-3">
                <strong className="text-[var(--text)]">{selectedEvent.name}</strong>
                <span className="text-[0.9rem] text-[var(--text-muted)]">{dateTime.format(new Date(selectedEvent.startsAt))}</span>
                <span className="text-[0.9rem] text-[var(--text-muted)]">{selectedEvent.venue}</span>
              </div>
            )}
          ...
          <section className={panelPadded}>
            ...
            {tickets.length === 0 ? (
              ...
              <div className="grid gap-3">
                {tickets.map((ticket) => (
                  <article className="grid grid-cols-[92px_1fr] items-center gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] p-3 max-[600px]:grid-cols-1" key={ticket.id}>
                    <img
                    ...
                    <div>
                      ...
                      <p className="mb-2 text-[var(--text-muted)]">{ticket.event.name}</p>
                      <code className="rounded-md bg-[var(--surface-elevated)] px-2 py-1 text-[0.78rem] text-[var(--accent)]">{ticket.code}</code>
                    </div>
                  </article>
                ))}
              </div>
              ...
              <div className="grid gap-2.5">
                {ticketHistory.length === 0 ? (
                  ...
                  ticketHistory.map((ticket) => (
                    <article className="flex items-center justify-between gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] p-3" key={ticket.id}>
                      <div>
                        <strong className="block text-[var(--text)]">{ticket.event.name}</strong>
                        <small className="text-[var(--text-muted)]">
                          {dateTime.format(new Date(ticket.event.startsAt))}
                        </small>
                      </div>
                      <span className={`inline-flex min-h-[28px] items-center rounded-full px-2.5 text-[0.75rem] font-[var(--weight-semibold)] uppercase ${ticket.status === "checked_in" ? "bg-[#dff7e8] text-[#14532d]" : ticket.status === "cancelled" ? "bg-[#ffe8df] text-[#8d2718]" : "bg-[var(--accent-soft)] text-[var(--accent)]"}`}>
