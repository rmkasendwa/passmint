...
export function useHostedEvents(
  ...
  const [state, setState] = useState<{
    ...
    error: "",
    loading: initialEvents === undefined,
    refreshing: false,
  });
    ...
    const refresh = async () => {
      ...
      setState((current) =>
        current.token === token
          ...
          : { token, events: [], error: "", loading: true, refreshing: true },
      );
      try {
        ...
        if (active)
          setState((current) => ({
            ...
            refreshing: false,
            error:
              "Unable to refresh your events. Any events shown are from the last successful update.",
          }));
      } finally {
        pending = false;
      }
  ...
  const current =
    state.token === token
      ? state
      : { events: [], error: "", loading: true, refreshing: true };
