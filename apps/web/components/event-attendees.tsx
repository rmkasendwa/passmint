...
export function EventAttendees({ eventId, token }: { eventId: string; token: string }) {
  ...
  useEffect(() => {
    ...
    setData(null);
    setError('');
    void api.eventAttendees(eventId, query.search, query.page, token)
      .then(value => { if (active) setData(value); })
      .catch(error => { if (active) setError(error?.message ?? 'Unable to load attendees.'); })
  ...
  function find(event: FormEvent) {
    ...
    setQuery(current => ({ search: search.trim(), page: 1, refresh: current.refresh + 1 }));
  }
  const button = 'min-h-11 rounded-lg border border-border bg-surface-muted px-4 text-text disabled:opacity-50';
  const labels = { issued: 'Issued', checked_in: 'Checked in', cancelled: 'Cancelled' };
  return <section className="grid min-w-0 gap-4 rounded-lg border border-border bg-surface-raised p-4.5" aria-labelledby="attendees-heading">
    <h2 id="attendees-heading" className="m-0 text-[1.55rem] text-text">Attendees</h2>
    <p className="m-0 text-text-muted">One row per ticket. Issued tickets do not confirm payment; payment verification is not yet available.</p>
    <form className="flex flex-wrap items-end gap-2" onSubmit={find}>
      <label className="grid min-w-0 flex-1 gap-2 text-text">Search by name or email
        <input className="min-h-11 w-full rounded-lg border border-border bg-surface-muted px-3 text-text" type="search" maxLength={100} value={search} onChange={event => setSearch(event.target.value)} />
      </label>
      <button className={button} type="submit">Search</button>
      <button className={button} type="button" onClick={() => setQuery(current => ({ ...current, refresh: current.refresh + 1 }))} disabled={loading}>Refresh</button>
    </form>
    {loading && <p role="status" className="m-0 text-text-muted">Loading attendees…</p>}
    {error && <p role="alert" className="m-0 text-text">{error}</p>}
    {data && <>
      {data.attendees.length === 0 ? <p role="status" className="m-0 text-text-muted">{query.search ? 'No attendees match this search.' : 'No attendees on this page.'}</p> :
      ...
      <nav className="flex flex-wrap items-center gap-3" aria-label="Attendee pages">
        <button className={button} type="button" disabled={query.page === 1} onClick={() => setQuery(current => ({ ...current, page: current.page - 1 }))}>Previous</button>
        <span className="text-text" role="status">Page {data.page} · {data.attendees.length} tickets shown</span>
