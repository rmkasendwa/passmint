...
`GET /events/:id/scan-metrics?day=YYYY-MM-DD` requires the owner or platform admin and returns `Cache-Control: no-store`. Drafts remain private to their owner. The optional day defaults to the current UTC date; malformed or impossible calendar dates return 400. Reports cover midnight inclusive to the following midnight exclusive, with 24 zero-filled UTC hourly buckets, generated from one database snapshot.

Daily fields are `day`, `attempts`, `accepted`, `failed`, `duplicates`, `timedScans`, nullable `averageDecisionMs`, nullable `peakCheckInHour`, `peakCheckIns`, `hourly` and `generatedAt`. Each hourly row has `hour` (ISO timestamp), counts and nullable average timing. Failed attempts include duplicates, cancelled-ticket/event rejections and unauthorized operators. Only known-ticket scan activities count; unknown codes, earlier unrecorded attempts and non-scan activity kinds do not. The peak uses accepted check-ins, with the earliest hour winning ties. Hourly throughput is a count per UTC hour; the current hour can be incomplete.

New scan records include nullable integer `decisionDurationMs`, measured with a monotonic server clock from service entry to the decision, including database/lock waits and accepted-ticket updates. It excludes camera reading, network travel, audit-write/commit time and QR response generation. Old null durations are excluded from the average; `timedScans` exposes coverage and zero-duration measurements count normally. The daily average is weighted by measured scans, not an average of hourly averages. Reports and CSV contain aggregate values only, without buyer, operator, device or QR credentials.

The dashboard exports the displayed report as 24 hourly CSV rows with explicit UTC timestamps, counts, timing coverage, nullable average timing and report generation time. Export is disabled during refresh or after a failed refresh. Apply the additive duration column and ticket event-ID index through the existing schema-sync process before running the updated API; no historical timings are backfilled.
