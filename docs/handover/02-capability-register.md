...
| Ticket scan activity | Implemented in source (13 September 2026) | Owner/admin attendee history shows issuance and paged scan attempts, including duplicates, cancellations and unauthorized operators. Records preserve operator ID/name and bounded browser-reported device information; legacy check-ins are identified without invented metadata. Transfer history in #24 remains pending the transfer feature |
| Scan performance metrics | Implemented in source (13 September 2026) | Owner/admin event report selects a UTC day and shows known-ticket attempts, accepted/failed/duplicate counts, hourly throughput, peak check-in hour and average measured server decision time. Visible reports refresh every 30 seconds; an accessible table and hourly CSV export share the displayed snapshot. Unknown QR codes and unrecorded historical attempts are excluded; legacy timings remain unknown |
