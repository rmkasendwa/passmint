...
| `/events`                  | Validated session and hosted events                                     | Filters, polling and mutations                         |
| `/reports`                 | Validated session and sales summary                                     | Refresh and interactive reports                        |
| `/events/[eventId]`        | Event, sales summary, attendees and scan metrics                        | Editing, pagination and report date changes            |
| `/events/new`              | Authenticated form and preview shell                                    | File selection, validation and submission              |
| `/check-in`                | Authenticated scanner controls and manual entry form                    | Camera access, QR decoding and scanning                |
...
The direct `/events/new` route renders the shared creation form on the server. The root `@modal` slot intercepts in-app links to that URL and displays the same form in a native modal dialog mounted in a body portal. Default and catch-all slot pages clear the overlay on navigation. Both entry points validate the server session.
...
Organizer URLs are `/events`, `/events/new`, `/events/[eventId]`, `/reports`, and `/check-in`. Legacy `/dashboard/*` URLs permanently redirect to their corresponding top-level paths. Login return destinations are validated and normalized by `organizerReturnPath`.

Browser-session restoration runs once per provider mount and reuses its pending promise. It must not depend on the top loader router object's identity: that object can change between renders and repeatedly trigger server actions and refreshes.
